...
*** web服务模型
    ...
    #+BEGIN_SRC ditaa :file ../img/web-server-model.png :cmdline -r :eval no-export
      +---------+     +----------------+     +----------------+     +---------------------+
...
*** MVC
    ...
    #+BEGIN_SRC ditaa :file ../img/web-mvc-model.png :cmdline -r :eval no-export
                                             +-----------+       +----------------+
...
*** 结构
    spring的结构如下：\\
    [[file:../img/spring_framework.gif]]
*** spring mvc
    spring mvc是对mvc模型的实现其一次请求处理过程如下：\\
    [[file:../img/spring-mvc-recquest-flow.jpg]]
** DI -- 依赖注入
